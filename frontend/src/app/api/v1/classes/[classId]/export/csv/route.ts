...
import { exportClassCSV } from '@/lib/services/export.service'
...
export async function GET(
  ...
  const limited = await rateLimitOrNull(req, 'export', 'csv')
  ...
  try {
    ...
    console.log(JSON.stringify({ event: 'export.route.xlsx.class', classId, organizationId: user.organizationId }))

    const xlsxBuffer = await exportClassCSV(classId, user.organizationId)

    return new NextResponse(new Uint8Array(xlsxBuffer), {
      status: 200,
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        'Content-Disposition': `attachment; filename="${className}_reports.xlsx"`,
