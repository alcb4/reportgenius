...
export default function CompactFilterBar({
  ...
  // Count active tests for indicator
  ...
  return (
    <div ref={panelRef} className="relative mb-3">
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm px-4 py-2 flex items-center w-full">
        ...
        <div className="w-1/2 pr-4 flex flex-wrap items-center gap-2">
          ...
          {(["gentle", "balanced", "direct"] as const).map((t) => (
            <div key={t} className="relative group">
              <button
                ...
                disabled={isReadOnly}
                className={`h-10 px-4 rounded-full text-xs font-medium transition border ${
                  filterState.tone === t
          ...
          <div className="flex flex-wrap gap-x-1.5 gap-y-1 items-center flex-1 min-w-0">
            {disciplines.length === 0 ? (
              ...
              disciplines.map((d) => (
                <span
                  key={d.id}
                  className="inline-flex items-center rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-700 whitespace-normal"
                >
                  {d.name}
                </span>
              ))
      ...
      {/* Expansion panel */}
      {open && (
        <div className="mt-1 bg-white rounded-lg border border-gray-200 shadow-md p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* LEFT — Tests */}
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              {!isReadOnly && (
                <button
                  ...
                  className="flex items-center gap-1 px-2.5 py-1 rounded-md border border-gray-200 text-xs font-medium text-gray-600 hover:border-indigo-300 hover:text-indigo-600 transition"
                >
                  Tests
                  <svg
                    className="w-3 h-3"
          ...
          <div>
            ...
            {!progressionData?.previousSession ? (
              ...
              <div className="space-y-1.5">
                ...
                {/* Allow negative toggle — global, always at bottom */}
                <label
                ...
                >
                  ...
                  />
                  <span className="text-xs text-gray-600">
                    Allow negative comments (declines)
                  </span>
                </label>
              </div>
            )}
